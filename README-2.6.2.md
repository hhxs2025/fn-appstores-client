...
[![版本](https://img.shields.io/badge/版本-v2.6.2-blue.svg)](https://gitee.com/hhxs2025/fn-appstores)
...
| **端口模式降级（v2.6.2）** | 通过 `FN_SOFT_START_MODE=port` 一键切回旧模式，兼容低版本系统 |
...
- **顶部轮播图**：展示平台宣传图（占70%宽度），**点击可查看大图**（v2.6.2 新增）
...
| **v2.6.2** | **飞牛统一网关**：应用改走 Unix Socket，通过 `/app/fn-appstores-client` 路径访问，不再监听 TCP 端口<br>**HTTPS 图片代理**：新增 `/api/proxy-image`，HTTPS 页面下 HTTP 图片自动走同源代理，解决浏览器混合内容拦截<br>**轮播图点击放大**：首页轮播图支持点击查看大图，支持左右切换、ESC 关闭<br>**图标与截图同步修复**：应用卡片、详情预览、快捷卡片图标统一走图片代理<br>**端口模式可降级**：通过 `FN_SOFT_START_MODE=port` 一键切回旧模式<br>**业务功能完全一致**：应用列表、安装、更新、向导、公告、源管理等 v2.6.1 的所有功能保持不变 |
...
<details>
<summary><b>升级到 v2.6.2 后桌面没有图标？（v2.6.2）</b></summary>
...
<details>
<summary><b>HTTPS 访问时轮播图/图标不显示？（v2.6.2）</b></summary>

v2.6.2 已通过客户端图片代理解决。如果仍不显示：

1. 打开浏览器 DevTools → Network，筛选 `proxy-image`
2. 若无请求：检查 `index.html` 是否已替换为 v2.6.2 版本
3. 若返回 403：图片域名不在白名单，检查该域名是否属于已配置的软件源
4. 若返回 500：客户端拉图失败，查看 `app.log` 中「代理图片失败」日志
5. 若返回 413：单张图片超过 10MB，调整 `IMAGE_PROXY_MAX_SIZE`
</details>

<details>
<summary><b>系统版本低于 1.2.0401 装不上？（v2.6.2）</b></summary>
...
<details>
<summary><b>打包时提示 `app.sock: no such device or address`？（v2.6.2）</b></summary>
...
<details>
<summary><b>fnpack 可视化编辑器"应用入口"页一片空白？（v2.6.2）</b></summary>
...
<details>
<summary><b>如何确认已成功接入统一网关？（v2.6.2）</b></summary>
