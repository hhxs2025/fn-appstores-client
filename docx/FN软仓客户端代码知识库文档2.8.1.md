...
> 本文档用于记录 FN软仓客户端 v2.8.1 的代码结构、核心逻辑和版本演进，便于后续开发和维护。
...
| **v2.8.0** | **应用评分系统** + **架构自动适配** + **评分徽章** + **字段 `arch` → `platform`** + **骨架屏** + **搜索防抖** + **卡片图标懒加载** + **一次性下载 + gzip 校验重试** + **进度显示自适应** | **已发布** |
| **v2.8.1** | **设置页新增建源指南**+**修复开发者页星球视图下搜索开发者功能无效的问题(搜索开发者时，被搜索对象白色光圈高亮，其余开发者星光淡化)** | **当前版本** |
